...
cap = {
    ...
    e.EV_KEY: [
        ...
        # 追加ボタン（既存コードより数値を大きくして
        # 末尾に配置し、既存ボタンの番号がズレないようにする）
        # ZL(デジタル) / ZR(デジタル) / Home
...
ui = UInput(
    events=cap,
    name="Switch Browser Virtual Controller",
    vendor=0x1234,
    product=0x5678,
    version=1,
    bustype=e.BUS_USB
...
def send_controller(data):
    ...
    with controller_lock:
        ...
        for index, keycode in button_mapping.items():
        ...
        # ----------------------------------------------------
        # ZL / ZR (アナログ値 + デジタルボタン)
        # 実機のGamepad APIではボタン17がZL、18がZRとして
        # 送られてくるためそのインデックスを使用する
        # ----------------------------------------------------
        ...
        ui.write(
            e.EV_KEY,
            e.BTN_TRIGGER_HAPPY1,
            1 if zl_pressed else 0
        )

        ui.write(
            e.EV_KEY,
            e.BTN_TRIGGER_HAPPY2,
            1 if zr_pressed else 0
        )
...
if __name__ == "__main__":
    ...
    print("=" * 65)
    print(
        " Switch PC Streaming Controller"
    )
    print("=" * 65)
    print()
